 ...
 * @param {*} getPrentIdFunc 父节点字段 默认 'getPrentIdFunc'
 */

export function handleTree<T>(data: T[], id: string = 'id', children: string = 'children', getPrentIdFunc: (arg0: T) => any): T[] {
  ...
  const pNode2ChildrenList = {}
  const tree: T[] = []

  for (const d of data) {
    const parentId = getPrentIdFunc(d)
    if (parentId === null || parentId === undefined) {
      tree.push(d)
      continue
    }
    if (pNode2ChildrenList[parentId] == null) {
      pNode2ChildrenList[parentId] = []
    }
    pNode2ChildrenList[parentId].push(d)
  }

  for (const t of tree) {
    adaptToChildrenList(t)
  }
  for (let list of Object.keys(pNode2ChildrenList)) {
    tree.push(...pNode2ChildrenList[list])
  }

  function adaptToChildrenList(o: T) {
    if (pNode2ChildrenList[o[config.id]] !== null) {
      o[config.children] = pNode2ChildrenList[o[config.id]]
      delete pNode2ChildrenList[o[config.id]]
    }
