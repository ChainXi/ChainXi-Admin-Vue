          ...
          <el-table-column label="字典类型" min-width="12%">
            <template #default="scope">
              <el-select v-model="scope.row.dictType" clearable filterable placeholder="请选择">
                <el-option v-for="dict in dictOptions" :key="dict.type" :label="dict.name" :value="dict.type">
...
import { getIndexMapDictTypeList } from '@/api/sys/dict/dict.type'
import basicInfoForm from './basicInfoForm.vue'
import genInfoForm from './genInfoForm.vue'
import { useTagsViewStore } from '@/store/modules/tagsView'

const tagsViewStore = useTagsViewStore()
const basicForm = useTemplateRef('basicInfo')
const genForm = useTemplateRef('genInfo')
...
function submitForm() {
  Promise.all([basicForm, genForm].map(getFormPromise)).then((res) => {
    ...
    if (validateResult) {
      const genTable = cloneDeep(unref(info))
...
onMounted(() => {
  ...
  if (tableId) {
    ...
    /** 查询字典下拉列表 */
    getIndexMapDictTypeList().then((response) => {
