...
export default defineComponent({
  ...
  setup() {
    ...
    const permissionStore = useRouterStore()

    const routers = computed(() => permissionStore.getRouters)

    const tabRouters = computed(() => unref(routers).filter((v) => v?.meta?.status === 0))
    ...
    onMounted(() => {
      if (unref(fixedMenu)) {
        ...
        if (children) {
          permissionStore.setMenuTabRouters(cloneDeep(children))
        }
      }
    })
    ...
    const tabClick = (item: AppRouteRecordRaw) => {
      ...
      if (item.children) {
        ...
        if (unref(showMenu)) {
          permissionStore.setMenuTabRouters(cloneDeep(item.children))
        }
      } else {
        push(item.path)
    ...
    return () => (
      ...
      >
          ...
          <div>
            {() => {
              return unref(tabRouters).map((v) => {
                const item = v as AppRouteRecordRaw
                return (
                  <div
                    class={[
                      `${prefixCls}__item`,
                  ...
                  >
                    ...
                    {unref(collapse) ? undefined : <p class="break-words mt-5px px-2px">{t(item.meta?.title)}</p>}
                  </div>
                )
              })
            }}
        ...
        {console.debug(unref(showMenu), unref(fixedMenu))}
        <Menu
          class={[
            '!absolute top-0',
            {
