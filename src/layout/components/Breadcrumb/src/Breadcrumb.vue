...
export default defineComponent({
  ...
  setup() {
    ...
    const getBreadcrumb = () => {
      const currentPath = currentRoute.value.matched.slice(-1)[0].path.replace(/\/:[^\/]+/g, '')
      levelList.value = filter<AppRouteRecordRaw>(unref(menuRouters), (node: AppRouteRecordRaw) => {
        return node.path.replace(/\/:[^/]+/g, '') === currentPath
      })
    }
    ...
    const renderBreadcrumb = () => {
      ...
      return breadcrumbList.map((v) => {
        ...
        return (
          <ElBreadcrumbItem class="cursor-pointer" to={v.component ? { path: v.path } : ''} key={v.name}>
